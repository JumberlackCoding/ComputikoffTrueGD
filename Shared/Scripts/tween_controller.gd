...
signal phase_and_scale_in_finished(target_node: Control)
signal phase_and_scale_out_finished(target_node: Control)
signal tween_color_finished(target_node: Control)
signal shake_finished(target_node: Control)

var tweens := {}
var z_indexes := {}

func _prepare_for_tween(target_node: Control) -> void:
...
func phase_and_scale_in(target_node: Control, duration_phase: float, duration_scale: float, start_scale: Vector2, end_scale: Vector2, scale_pivot_ratio: Vector2,
    ...
    var end_phase: int = 1
    var phase_transform_property: String = "modulate:a"
    var scale_transform_property: String = "offset_transform_scale"
    ...
    await tween.finished
    _cleanup_for_tween(target_node)
    phase_and_scale_in_finished.emit(target_node)

func phase_and_scale_out(target_node: Control, duration_phase: float, duration_scale: float, start_scale: Vector2, end_scale: Vector2, scale_pivot_ratio: Vector2,
                         transition_type: Tween.TransitionType = Tween.TRANS_LINEAR, ease_type: Tween.EaseType = Tween.EASE_IN_OUT) -> void:
    _tween_safety_check(target_node)
    _prepare_for_tween(target_node)
    ...
    tween.set_trans(transition_type)
    tween.set_ease(ease_type)
    tween.tween_property(target_node, phase_transform_property, end, duration_phase).from(start)
    tween.parallel().tween_property(target_node, scale_transform_property, end_scale, duration_scale).from(start_scale)
    await tween.finished
    target_node.visible = false
    _cleanup_for_tween(target_node)
    phase_and_scale_out_finished.emit(target_node)

func shake(target_node: Control, direction: Vector2, position_by_ratio: bool, repetition_duration: float, repetitions: int, intensity: float,
              transition_type: Tween.TransitionType = Tween.TRANS_QUINT, ease_type: Tween.EaseType = Tween.EASE_IN_OUT) -> void:
    _prepare_for_tween(target_node)
    ...
    for rep in range(repetitions):
        ...
        var end_1: Vector2 = starting_pos + (direction.normalized() * intensity)
        ...
        await tween.finished
        _tween_safety_check(target_node)
        var start_2: Vector2 = starting_pos + (direction.normalized() * intensity)
        var end_2: Vector2 = starting_pos - (direction.normalized() * intensity)
        ...
        await tween.finished
        _tween_safety_check(target_node)
        var start_3: Vector2 = starting_pos - (direction.normalized() * intensity)
    ...
    _cleanup_for_tween(target_node)
    shake_finished.emit(target_node)
